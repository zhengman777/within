...
class Shadow(pygmi.Object):
    ...
    def update(self):
        if self.owner == oBoy:
            ...
            self.sprite.setFlipped(self.owner.sprite.flipx,self.owner.sprite.flipy)
            if self.sprite.flipx == 0:
                self.x = self.owner.x-2
            elif self.sprite.flipx == 1:
                self.x = self.owner.x-4
...
class Character(pygmi.Object):
    ...
    def __init__(self,x,y):
        ...
        self.kickAnim = 0
        self.datkAnim = 0
        sprBoyIdle = pygmi.Sprite("img/char/boy_idle",-18,-64,64,64)
        sprBoyIdle.setFrameTime(30)
        sprBoyWalk = pygmi.Sprite("img/char/boy_walk",-18,-66,64,66)
        sprBoyWalk.setFrameTime(8)
        sprBoyRun = pygmi.Sprite("img/char/boy_run",-22,-70,64,70)
        sprBoyRun.setFrameTime(5)
        sprBoyPunch1 = pygmi.Sprite("img/char/boy_punch1",-18,-64,64,64)
        sprBoyPunch1.setFrameTime(2)
        sprBoyPunch2 = pygmi.Sprite("img/char/boy_punch2",-20,-64,64,64)
        sprBoyPunch2.setFrameTime(2)
        sprBoyKick = pygmi.Sprite("img/char/boy_kick",-18,-64,64,64)
        sprBoyKick.setFrameTime(2)
        sprBoyDatk = pygmi.Sprite("img/char/boy_datk",-20,-66,64,66)
        sprBoyDatk.setFrameTime(3)
        self.boy = {'idle':sprBoyIdle,'walk':sprBoyWalk,'run':sprBoyRun,'punch1':sprBoyPunch1,'punch2':sprBoyPunch2,'kick':sprBoyKick,'datk':sprBoyDatk}
            ...
            if self.airborne == 0:
                if self.moving == 0:
                    ...
                    if self.attacking == 0:
                        ...
                        self.running = 0
        if key == K_k:
            if self.airborne == 0 and self.moving == 0 and self.attacking == 0:
    ...
    def update(self):
        ...
        if self.datkAnim == 0:
            ...
            if self.xSpeed == 0 and self.ySpeed == 0:
                self.moving = 0
            if self.moving == 0 and self.attacking == 0:
